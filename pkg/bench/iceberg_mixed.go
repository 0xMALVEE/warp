...
func (b *IcebergMixed) Prepare(ctx context.Context) error {
	...
	b.UpdateStatus(fmt.Sprintf("Loading dataset info: %d namespaces, %d tables, %d views",
	...
	_, err := b.RestClient.GetNamespace(ctx, catalog, b.namespaces[0].Path)
	if err != nil {
		return fmt.Errorf("cannot access catalog: %w", err)
	}

	b.UpdateStatus("Preparation complete - dataset exists")
	return nil
}
